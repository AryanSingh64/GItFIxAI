          ...
          <tbody className="divide-y divide-white/5">
            {fixes.map((fix, index) => (
              <tr key={index} className="hover:bg-white/5 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span
                    className={`px-2 py-1 rounded text-xs font-bold ${
                      fix.type === 'LINTING' || fix.type === 'LINT_ERROR'
                        ? 'bg-yellow-500/10 text-yellow-500'
                        : fix.type === 'SYNTAX' || fix.type === 'BUILD_ERROR'
                        ? 'bg-red-500/10 text-red-500'
                        : fix.type === 'SECURITY_VULNERABILITY'
                ...
                <td className="px-6 py-4 text-gray-300 max-w-xs truncate" title={fix.commit}>
                  {fix.commit}
                </td>
                <td className="px-6 py-4">
                  {fix.status === 'FIXED' ? (
