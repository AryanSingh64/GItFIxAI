        ...
        <div className="space-y-6">
          {runs.map((run, index) => (
            <div key={index} className="relative pl-12">
              <div
                className={`absolute left-[11px] top-1 w-3 h-3 rounded-full border-2 border-surface ${
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-secondary mt-1">{run.message || run.repo_url}</p>
                </div>
                <span className="text-xs text-secondary font-mono bg-black/20 px-2 py-1 rounded">
                  {run.timestamp || run.created_at?.slice(0, 10)}
                </span>
              </div>
            </div>
          ))}
